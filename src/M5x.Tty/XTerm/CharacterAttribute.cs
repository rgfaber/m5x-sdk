...
namespace M5x.Tty.XTerm
{
    ...
    public static class CharacterAttribute
    {
        ...
        public static string ToSGR(int attribute)
        {
            ...
            if (ca.HasFlag(FLAGS.BOLD)) result += ";1";
            if (ca.HasFlag(FLAGS.UNDERLINE)) result += ";4";
            if (ca.HasFlag(FLAGS.BLINK)) result += ";5";
            if (ca.HasFlag(FLAGS.INVERSE)) result += ";7";
            if (ca.HasFlag(FLAGS.INVISIBLE)) result += ";8";

            var fg = (attribute >> 9) & 0x1ff;

            if (fg != Renderer.Renderer.DefaultColor)
            {
                if (fg > 16)
                {
                    result += $";38;5;{fg}";
            ...
            if (bg != Renderer.Renderer.DefaultColor)
            {
                if (bg > 16)
                {
                    result += $";48;5;{bg}";
