...
using System.Text;
using M5x.Tty.XTerm.InputHandlers;
...
namespace M5x.Tty.XTerm
{
    public class Terminal
    {
        ...
        private int refreshStart = int.MaxValue;
        ...
        {
            Delegate = terminalDelegate ?? new SimpleTerminalDelegate();
        ...
        /// </summary>
        public ITerminalDelegate Delegate { get; }

        /// <summary>
        ///     Gets the control codes for the terminal
        /// </summary>
        public ControlCodes ControlCodes { get; }
        ...
        {
            Title = text;
            Delegate.SetTerminalTitle(this, text);
        }
        ...
        internal void PopTitle()
        {
            ...
            Delegate.SetTerminalTitle(this, Title);
        }
        ...
        {
            IconTitle = text;
            Delegate.SetTerminalIconTitle(this, text);
        }
        ...
        internal void PopIconTitle()
        {
            ...
            Delegate.SetTerminalIconTitle(this, IconTitle);
        }
        ...
        {
            Delegate.Send(Encoding.UTF8.GetBytes(txt));
        }
        ...
        public void SendResponse(params object[] args)
        {
            ...
            for (var i = 0; i < args.Length; i++)
                ...
                else if (args[i] is byte[] ba) len += ba.Length - 1;
            ...
            for (var i = 0; i < args.Length; i++)
            {
                ...
                {
                    foreach (var bab in ba) buffer[bufferIndex++] = bab;
                    bufferIndex--;
                }
                else
                {
        ...
        internal void Scroll(bool isWrapped = false)
        {
            ...
            if (buffer.ScrollTop == 0)
            {
                ...
                if (!willBufferBeTrimmed)
                {
                    ...
                    if (!userScrolling) buffer.YDisp++;
                }
                else
                {
                    // When the buffer is full and the user has scrolled up, keep the text
                    // stable unless ydisp is right at the top
                    if (userScrolling) buffer.YDisp = Math.Max(buffer.YDisp - 1, 0);
                }
            }
            else
            {
                ...
                if (scrollRegionHeight > 1) buffer.Lines.ShiftElements(topRow + 1, scrollRegionHeight - 1, -1);
            ...
            // scrolling.
            if (!userScrolling) buffer.YDisp = buffer.YBase;
        ...
        /// <param name="suppressScrollEvent">
        ///     Don't emit the scroll event as scrollLines. This is use to avoid unwanted
        ///     events being handled by the viewport when the event was triggered from the viewport originally.
        /// </param>
        public void ScrollLines(int disp, bool suppressScrollEvent = false)
        {
            if (disp < 0)
            {
                if (Buffer.YDisp == 0) return;
            ...
            if (!suppressScrollEvent) Scrolled?.Invoke(this, Buffer.YDisp);
        ...
        public void SendEvent(int buttonFlags, int x, int y)
        {
            switch (MouseProtocol)
            {
                case MouseProtocolEncoding.X10:
                    SendResponse(ControlCodes.CSI, "M", (byte) (buttonFlags + 32), (byte) Math.Min(255, 32 + x + 1),
                        (byte) Math.Min(255, 32 + y + 1));
                    break;
                ...
                case MouseProtocolEncoding.UTF8:
                    ...
                    EncodeMouseUtf(utf8, buttonFlags + 32);
                    EncodeMouseUtf(utf8, x + 33);
                    EncodeMouseUtf(utf8, y + 33);
        ...
        public void CursorDown(int rows)
        {
            ...
            if (buffer.Y > bottom) bottom = buffer.Rows - 1;
        ...
        public void CursorForward(int cols)
        {
            ...
            if (buffer.X > right) right = buffer.Cols - 1;

            buffer.X += cols;
            if (buffer.X > right) buffer.X = right;
        }
        ...
        public void CursorBackward(int cols)
        {
            ...
            if (buffer.X < left) left = 0;
            buffer.X -= cols;

            if (buffer.X < left) buffer.X = left;
        }
        ...
        {
            var buffer = Buffer;
            while (tabs-- != 0) buffer.X = buffer.PreviousTabStop();
        }
        ...
        {
            var buffer = Buffer;
            if (Options.ConvertEol) buffer.X = MarginMode ? buffer.MarginLeft : 0;
        ...
        public void LineFeedBasic()
        {
            ...
            // If the end of the line is hit, prevent this action from wrapping around to the next line.
            if (buffer.X >= buffer.Cols) buffer.X -= 1;
        ...
        ///     - Parameter limitCols: by default it is true, but the reverseWraparound mechanism in Backspace needs `x` to go
        ///     beyond.
        /// </summary>
        ...
        public void Backspace()
        {
            ...
            else if (ReverseWraparound)
            {
                if (buffer.X <= left)
                {
                    ...
                    {
                        if (!MarginMode) buffer.Lines[buffer.Y + buffer.YBase].IsWrapped = false;
            ...
            {
                if (buffer.X < left) // This compensates for the scenario where backspace is supposed to move one step
                    // backwards if the "x" position is behind the left margin.
                    // Test BS_MovesLeftWhenLeftOfLeftMargin
                    buffer.X--;
                else if (
                        buffer.X >
                        left) // If we have not reached the limit, we can go back, otherwise stop at the margin
        ...
        public void DeleteChars(int charsToDelete)
        {
            ...
            buffer.Lines[buffer.Y + buffer.YBase].DeleteCells(buffer.X, charsToDelete,
                MarginMode ? buffer.MarginRight : buffer.Cols - 1, new CharData(EraseAttr()));
                ...
                if (buffer.X >= buffer.MarginLeft && buffer.X <= buffer.MarginRight)
                {
                    ...
                    while (rowsToDelete-- > 0)
                    {
                        for (var i = 0; i < rowCount; i++)
                        {
                            ...
                            if (src != null) dst.CopyFrom(src, buffer.MarginLeft, buffer.MarginLeft, columnCount);
                        }

                        var last = buffer.Lines[row + rowCount];
                        last?.Fill(new CharData(eraseAttr), buffer.MarginLeft, columnCount);
                    }
                }
            }
            else
        ...
        public void InsertColumn(int columns)
        {
            ...
            for (var row = buffer.ScrollTop; row < buffer.ScrollBottom; row++)
            {
                var line = buffer.Lines[row + buffer.YBase];
                // TODO:is this the right filldata?
                line.InsertCells(buffer.X, columns, MarginMode ? buffer.MarginRight : buffer.Cols - 1,
                    CharData.WhiteSpace);
