...
namespace M5x.Tty.XTerm
{
    ...
    public class SelectionService
    {
        ...
        private readonly ustring nullString;
        private readonly ustring spaceString;
        ...
        public SelectionService(Terminal terminal)
        {
            ...
            nullString = ustring.Make(CharData.Null.Rune);
            spaceString = ustring.Make(" ");
        }
        ...
        public bool Active
        {
            ...
            set
            {
                var oldState = active;
                active = value;
                if (oldState != value) SelectionChanged?.Invoke();
            }
        }
        ...
        public void ShiftExtend(int row, int col)
        {
            ...
            if (comparer.Compare(Start, End) < 0)
            {
                ...
                if (comparer.Compare(newEnd, Start) < 0) // yes, swap Start and End
        ...
        public void SelectWordOrExpression(int col, int row)
        {
            ...
            if (chr.IsNullChar())
            {
                SimpleScanSelection(col, row, ch => { return ch.IsNullChar(); });
            }
            else
            {
                if (isLetterOrChar(chr))
                {
                    SimpleScanSelection(col, row,
                        ch => { return isLetterOrChar(ch) || ch.MatchesRune(CharData.Period); });
                }
                else
                {
                    if (chr.MatchesRune(CharData.WhiteSpace))
                        SimpleScanSelection(col, row, ch => { return ch.MatchesRune(CharData.WhiteSpace); });
                    else if (chr.MatchesRune(CharData.LeftBrace) || chr.MatchesRune(CharData.LeftBracket) ||
                             chr.MatchesRune(CharData.LeftParenthesis))
                        BalancedSearchForward(col, row);
                    else if (chr.MatchesRune(CharData.RightBrace) || chr.MatchesRune(CharData.RightBracket) ||
                             chr.MatchesRune(CharData.RightParenthesis))
                        BalancedSearchBackward(col, row);
                    else // For other characters, we just stop there
                        Start = End = new Point(col, row + terminal.Buffer.YDisp);
                }
            }
        ...
        public Line[] GetSelectedLines()
        {
            ...
            if (start.Y < 0 || start.Y > terminal.Buffer.Lines.Length) return Array.Empty<Line>();

            if (end.Y >= terminal.Buffer.Lines.Length) end.Y = terminal.Buffer.Lines.Length - 1;
        ...
        private Line[] GetSelectedLines(Point start, Point end)
        {
            ...
            // get the first line
            var bufferLine = buffer.Lines[start.Y];
            ...
            while (line < end.Y)
            {
                ...
                else
                {
                    ...
                    if (!isWrapped) blanks.Add(LineFragment.NewLine(line - 1));
        ...
        private void BalancedSearchForward(int col, int row)
        {
            ...
            for (var line = row; line < terminal.Rows; line++)
            {
                for (var colIndex = startCol; colIndex < terminal.Cols; colIndex++)
                {
        ...
        private void BalancedSearchBackward(int col, int row)
        {
            ...
            for (var line = row; line > 0; line--)
            {
                for (var colIndex = startCol; colIndex > 0; colIndex--)
                {
                    var p = new Point(colIndex, line);
                    var ch = buffer.GetChar(colIndex, line);
