...
namespace M5x.Tty.XTerm
{
    ...
    public class BufferLine
    {
        ...
        private string DebuggerDisplay => TranslateToString(true).ToString();
        ...
        {
            return data[index].Code != 0 || data[index].Attribute != CharData.DefaultAttr;
        }
        ...
        public ustring TranslateToString(bool trimRight = false, int startCol = 0, int endCol = -1)
        {
            if (endCol == -1)
                endCol = data.Length;
            if (trimRight) // make sure endCol is not before startCol if we set it to the trimmed length
                endCol = Math.Max(Math.Min(endCol, GetTrimmedLength()), startCol);

            var runes = new Rune [endCol - startCol];
