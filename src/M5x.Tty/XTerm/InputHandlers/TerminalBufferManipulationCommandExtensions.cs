...
namespace M5x.Tty.XTerm.InputHandlers
{
    ...
    internal static class TerminalBufferManipulationCommandExtensions
    {
        ...
        public static void csiDECFRA(this Terminal terminal, params int[] pars)
        {
            ...
            {
                var fillChar = ' ';
                if (pars.Length > 0) fillChar = (char) pars[0];

                for (var row = rect.top; row <= rect.bottom; row++)
                {
                    var line = buffer.Lines[row + buffer.YBase];
                    for (var col = rect.left; col <= rect.right; col++)
                        line[col] = new CharData(terminal.CurAttr, fillChar, 1, fillChar);
                }
            }
        }
        ...
        public static void csiDECCRA(this Terminal terminal, int[] pars, string collect)
        {
            ...
            if (collect == "$")
            {
                ...
                parArray[2] =
                    pars.Length > 3 && pars[2] != 0 ? pars[2] : buffer.Rows - 1; // Pbs default to last line of page
                parArray[3] =
                    pars.Length > 4 && pars[3] != 0 ? pars[3] : buffer.Cols - 1; // Prs defaults to last column
                ...
                if (parArray[4] == parArray[7] && parArray[4] == 1)
                {
                    ...
                    if (rect.valid)
                    {
                        ...
                        for (var row = rect.top; row <= rect.bottom; row++)
                        {
                            ...
                            for (var col = rect.left; col <= cright; col++) lineCopy.Add(line[col]);
                            lines.Add(lineCopy);
                        }

                        for (var row = 0; row <= rect.bottom - rect.top; row++)
                        {
                            if (row + rowTarget >= buffer.Rows) break;

                            var line = buffer.Lines[row + rowTarget + buffer.YBase];
                            var lr = lines[row];
                            for (var col = 0; col <= cright - rect.left; col++)
                            {
                                if (col >= buffer.Cols) break;
        ...
        /// </summary>
        private static (bool valid, int top, int left, int bottom, int right) GetRectangleFromRequest(Buffer buffer,
            bool originMode, int start, int[] pars)
        {
            ...
            bool originMode, int top, int left, int bottom, int right)
        {
            ...
            if (right < 0) right = buffer.Cols;
            if (right > buffer.Cols) right = buffer.Cols;
            if (bottom > buffer.Rows) bottom = buffer.Rows;
            if (originMode)
            {
            ...
            if (top > bottom || left > right) return (false, 0, 0, 0, 0);
