...
namespace M5x.Tty.XTerm.InputHandlers
{
    // 
    ...
    internal class InputHandler
    {
        ...
        public InputHandler(Terminal terminal)
        {
            ...
            parser.SetEscHandlerFallback((collect, flag) => { terminal.Error("Unknown ESC code", collect, flag); });
            parser.SetExecuteHandlerFallback(code => { terminal.Error("Unknown EXECUTE code", code); });
            parser.SetOscHandlerFallback(
                (identifier, data) => { terminal.Error("Unknown OSC code", identifier, data); });
            ...
            {
                if (collect == "") terminal.csiDECSTBM(pars);
            });
            ...
            parser.SetExecuteHandler(11,
                terminal.LineFeedBasic); // VT Vertical Tab - ignores auto-new-line behavior in ConvertEOL
        ...
        private void RepeatPrecedingCharacter(int[] pars)
        {
            ...
            var cd = buffer.X - 1 < 0 ? new CharData(CharData.DefaultAttr) : line[buffer.X - 1];
        ...
        private void EraseInDisplay(int[] pars)
        {
            ...
            switch (p)
            {
                case 0:
                    ...
                    for (; j < terminal.Rows; j++) ResetBufferLine(j);
                ...
                case 1:
                    ...
                    if (buffer.X + 1 >=
                        terminal.Cols) // Deleted entire previous line. This next line can no longer be wrapped.
                        buffer.Lines[j + 1].IsWrapped = false;
                    while (j-- != 0) ResetBufferLine(j);
        ...
        {
            var param = Math.Max(pars.Length > 0 ? pars[0] : 1, 1);
            var buffer = terminal.Buffer;
            while (param-- != 0)
        ...
        private void InsertChars(int[] pars)
        {
            ...
            buffer.Lines[buffer.Y + buffer.YBase].InsertCells(
                ...
                terminal.MarginMode ? buffer.MarginRight : buffer.Cols - 1,
                cd);
        ...
        private unsafe void Print(byte* data, int start, int end)
        {
            ...
            {
                int code;
                var bufferValue = readingBuffer.GetNext();
                ...
                if (
                    code < 127 &&
                    charset != null) // MIGUEL-FIXME - this is broken for dutch cahrset that returns two letters "ij", need to figure out what to do
                ...
                if (buffer.X + chWidth - 1 > right)
                {
                    ...
                    if (wrapAroundMode)
                    {
                        ...
                        if (buffer.Y >= buffer.ScrollBottom)
                            terminal.Scroll(true);
                        else // The line already exists (eg. the initial viewport), mark it as a
                            // wrapped line
                ...
                if (insertMode)
                {
                    ...
                    bufferRow.InsertCells(buffer.X, chWidth, terminal.MarginMode ? buffer.MarginRight : cols - 1,
                        empty);
