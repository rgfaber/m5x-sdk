            ...
            if (pars.Length == 1)
            {
                ...
                switch (pars[0])
                {
                    ...
                    case 7:
                        ...
                        return;
                    case 15:
                        response = terminal.Delegate.WindowCommand(terminal,
                            WindowManipulationCommand.ReportSizeOfScreenInPixels);
                        if (response == null) response = $"{terminal.ControlCodes.CSI}5;768;1024t";

                        terminal.SendResponse(response);
                        return;
                    case 16:
                        response = terminal.Delegate.WindowCommand(terminal,
                            WindowManipulationCommand.ReportCellSizeInPixels);
                        if (response == null) response = $"{terminal.ControlCodes.CSI}6;16;10t";
                    ...
                    case 18:
                        response = terminal.Delegate.WindowCommand(terminal,
                            WindowManipulationCommand.ReportScreenSizeCharacters);
                        if (response == null)
