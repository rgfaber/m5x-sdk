...
namespace M5x.Tty.Core
{
    ...
    /// <summary>
    ///     Provides text formatting capabilities for console apps. Supports, hotkeys, horizontal alignment, multiple lines,
    ///     and word-based line wrap.
    /// </summary>
    public class TextFormatter
    {
        ...
        private List<ustring> lines = new();
        private Size size;
            ...
            set
            {
                ...
                if (text.RuneCount > 0 &&
                    (Size.Width == 0 || Size.Height == 0 ||
                     Size.Width !=
                     text.RuneCount)) // Provide a default size (width = length of longest line, height = 1)
        ...
        /// <summary>
        ///     The specifier character for the hotkey (e.g. '_'). Set to '\xffff' to disable hotkey support for this View
        ///     instance. The default is '\xffff'.
        /// </summary>
        public Rune HotKeySpecifier { get; set; } = 0xFFFF;
        ...
        /// <summary>
        ///     Gets or sets whether the <see cref="TextFormatter" /> needs to format the text when
        ///     <see cref="Draw(Rect, Attribute, Attribute)" /> is called.
        ...
        private static ustring StripCRLF(ustring str)
        {
        ...
        private static ustring ReplaceCRLFWithSpace(ustring str)
        {
            var runes = str.ToRuneList();
            for (var i = 0; i < runes.Count; i++)
                switch (runes[i])
        ...
        public static List<ustring> WordWrap(ustring text, int width)
        {
            ...
            if (ustring.IsNullOrEmpty(text)) return lines;
            ...
            if (start < text.RuneCount) lines.Add(ustring.Make(runes.GetRange(start, runes.Count - start)));
        ...
        {
            if (width < 0) throw new ArgumentOutOfRangeException("Width cannot be negative.");
            if (ustring.IsNullOrEmpty(text)) return text;

            var runes = text.ToRuneList();
            var slen = runes.Count;
            if (slen > width) return ustring.Make(runes.GetRange(0, width));

            if (talign == TextAlignment.Justified) return Justify(text, width);
            return text;
        }
        ...
        /// <param name="wordWrap">
        ///     If <c>true</c>, the text will be wrapped to new lines as need. If <c>false</c>, forces text to
        ///     fit a single line. Line breaks are converted to spaces. The text will be clipped to <c>width</c>
        /// </param>
        ...
        public static List<ustring> Format(ustring text, int width, TextAlignment talign, bool wordWrap)
        {
            ...
            var lineResult = new List<ustring>();
            ...
            for (var i = 0; i < runeCount; i++)
            {
                ...
                if (c == '\n')
                {
                    ...
                    foreach (var line in wrappedLines) lineResult.Add(ClipAndJustify(line, width, talign));
                    if (wrappedLines.Count == 0) lineResult.Add(ustring.Empty);
                    lp = i + 1;
                }
            }
        ...
        {
            var result = Format(text, width, TextAlignment.Left, true);
            return result.Count;
        }
        ...
        public static bool FindHotKey(ustring text, Rune hotKeySpecifier, bool firstUpperCase, out int hotPos,
            out Key hotKey)
        {
        ...
        public ustring ReplaceHotKeyWithTag(ustring text, int hotPos)
        {
            // Set the high bit
            var runes = text.ToRuneList();
            if (Rune.IsLetterOrNumber(runes[hotPos])) runes[hotPos] = new Rune(runes[hotPos] | HotKeyTagMask);
            return ustring.Make(runes);
        }
        ...
        /// <summary>
        ///     Draws the text held by <see cref="TextFormatter" /> to <see cref="Application.Driver" /> using the colors
        ///     specified.
        /// </summary>
        ...
        public void Draw(Rect bounds, Attribute normalColor, Attribute hotColor)
        {
            ...
            if (ustring.IsNullOrEmpty(text)) return;
            ...
            for (var line = 0; line < Lines.Count; line++)
            {
                ...
                for (var col = bounds.Left; col < bounds.Left + bounds.Width; col++)
                {
                    ...
                    var rune = (Rune) ' ';
                    if (col >= x && col < x + runes.Length) rune = runes[col - x];
                    if ((rune & HotKeyTagMask) == HotKeyTagMask)
                    {
                        if (textAlignment == TextAlignment.Justified) CursorPosition = col - bounds.Left;
                        Application.Driver?.SetAttribute(hotColor);
                        Application.Driver?.AddRune(rune & ~HotKeyTagMask);
                        Application.Driver?.SetAttribute(normalColor);
                    }
                    else
                    {
