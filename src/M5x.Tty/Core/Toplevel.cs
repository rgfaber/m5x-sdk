...
namespace M5x.Tty.Core
{
    ...
    ///         <see cref="Application.Init(ConsoleDriver, IMainLoopDriver)" />.
    ///         The application Toplevel can be accessed via <see cref="Application.Top" />. Additional Toplevels can be
    ///         created
    ...
    public class Toplevel : View
    {
        ...
        ///     <see cref="Application.Run()" />(topLevel)`.
        /// </summary>
        ...
        /// <summary>
        ///     Called from <see cref="Application.Begin(Toplevel)" /> before the <see cref="Toplevel" /> is redraws for the first
        ///     time.
        /// </summary>
        ...
        /// <summary>
        ///     Called from <see cref="Application.RunLoop" /> after the <see cref="Toplevel" /> has entered it's first iteration
        ///     of the loop.
        /// </summary>
        ...
        {
            return new(new Rect(0, 0, Driver.Cols, Driver.Rows));
        }
        ...
        private IEnumerable<View> GetToplevelSubviews(bool isForward)
        {
            if (SuperView == null) return null;

            var views = new HashSet<View>();

            foreach (var v in SuperView.Subviews) views.Add(v);

            return isForward ? views : views.Reverse();
        }

        private void FocusNearestView(IEnumerable<View> views)
        {
        ...
        {
            if (this == Application.Top) AddMenuStatusBar(view);
            base.Add(view);
        }

        internal void AddMenuStatusBar(View view)
        {
            if (view is MenuBar) MenuBar = view as MenuBar;
            if (view is StatusBar) StatusBar = view as StatusBar;
        }

        /// <inheritdoc />
        public override void Remove(View view)
        {
            if (this is Toplevel toplevel && toplevel.MenuBar != null) RemoveMenuStatusBar(view);
            base.Remove(view);
        }
        ...
        {
            EnsureVisibleBounds(top, top.Frame.X, top.Frame.Y, out var nx, out var ny);
            if ((nx != top.Frame.X || ny != top.Frame.Y) && top.LayoutStyle == LayoutStyle.Computed)
            {
                if (top.X is Pos.PosAbsolute && top.Bounds.X != nx) top.X = nx;
                if (top.Y is Pos.PosAbsolute && top.Bounds.Y != ny) top.Y = ny;
            }
