...
namespace M5x.Tty.Core
{
    ...
    ///         is determined when the View is initizlied. To create a View using Absolute layout, call a constructor that
    ///         takes a
    ///         Rect parameter to specify the absolute position and size (the <c>View.<see cref="Frame " /></c>)/. To create a
    ///         View
    ///         using Computed layout use a constructor that does not take a Rect parametr and set the X, Y, Width and Height
    ///         properties on the view. Both approaches use coordinates that are relative to the container they are being added
    ///         to.
    ///     </para>
    ///     <para>
    ...
    public class View : Responder, ISupportInitializeNotification
    {
        ...
        /// <remarks>
        ///     This constructor intitalize a View with a <see cref="LayoutStyle" /> of <see cref="LayoutStyle.Absolute" />. Use
        ///     <see cref="View()" /> to
        ...
        ///         dynamically control the size and location of the view.
        ...
        /// <summary>
        ///     Gets or sets the HotKey defined for this view. A user pressing HotKey on the keyboard while this view has focus
        ///     will cause the Clicked event to fire.
        /// </summary>
        public Key HotKey
        {
        ...
        /// <summary>
        ///     Gets or sets the specifier character for the hotkey (e.g. '_'). Set to '\xffff' to disable hotkey support for this
        ///     View instance. The default is '\xffff'.
        /// </summary>
        ...
        public int TabIndex
        {
            ...
            set
            {
                ...
                if (SuperView?.tabIndexes == null || SuperView?.tabIndexes.Count == 1)
                {
                    tabIndex = 0;
                    return;
                }
        ...
        /// <summary>
        ///     This only be <c>true</c> if the <see cref="CanFocus" /> is also <c>true</c> and the focus can be avoided by setting
        ///     this to <c>false</c>
        /// </summary>
        public bool TabStop
        {
        ...
        ///     LayoutSubviews does not change the <see cref="Frame" />. If the style is <see cref="LayoutStyle.Computed" /> the
        ///     <see cref="Frame" /> is updated using
        ...
        /// <summary>
        ///     Gets or sets the X position for the view (the column). Only used whe <see cref="LayoutStyle" /> is
        ///     <see cref="LayoutStyle.Computed" />.
        /// </summary>
        /// <value>The X Position.</value>
        /// <remarks>
        ///     If <see cref="LayoutStyle" /> is <see cref="LayoutStyle.Absolute" /> changing this property has no effect and its
        ...
        /// <summary>
        ///     Gets or sets the Y position for the view (the row). Only used whe <see cref="LayoutStyle" /> is
        ///     <see cref="LayoutStyle.Computed" />.
        /// </summary>
        ...
        /// <summary>
        ///     Gets or sets the height of the view. Only used whe <see cref="LayoutStyle" /> is
        ///     <see cref="LayoutStyle.Computed" />.
        /// </summary>
        ...
        /// is
        /// <see cref="LayoutStyle.Absolute" />
        /// changing this property has no effect and its value is indeterminate.
        public Dim Height
        {
            get => height;
            set
        ...
        /// <summary>
        ///     This derived from <see cref="ISupportInitializeNotification" /> to allow notify all the views that are beginning
        ///     initialized.
        /// </summary>
        ...
        /// <summary>
        ///     This derived from <see cref="ISupportInitializeNotification" /> to allow notify all the views that are ending
        ///     initialized.
        /// </summary>
        public void EndInit()
        {
        ...
        private bool ValidatePosDim(object oldvalue, object newValue)
        {
            if (!IsInitialized || layoutStyle == LayoutStyle.Absolute || oldvalue == null ||
                oldvalue.GetType() == newValue.GetType() || this is Toplevel) return true;
        ...
        public virtual void Add(View view)
        {
            if (view == null)
                return;
            if (subviews == null) subviews = new List<View>();
            if (tabIndexes == null) tabIndexes = new List<View>();
            subviews.Add(view);
            tabIndexes.Add(view);
            view.SuperView = this;
            if (view.CanFocus)
        ...
        /// <summary>
        ///     Converts a view-relative (col,row) position to a screen-relative position (col,row). The values are optionally
        ...
        /// <param name="clipped">
        ///     Whether to clip the result of the ViewToScreen method, if set to <c>true</c>, the rcol, rrow
        ///     values are clamped to the screen (terminal) dimensions (0..TerminalDim-1).
        /// </param>
        ...
        {
            if (SuperView == null) return new Point(x - Frame.X, y - frame.Y);
        ...
        /// </summary>
        /// <returns>
        ///     The existing driver's clip region, which can be then re-eapplied by setting <c><see cref="Driver" />.Clip</c>
        ...
        {
            var hotkeySpec = HotKeySpecifier == (Rune) 0xffff ? '_' : HotKeySpecifier;
        ...
        /// <remarks>
        ///     <para>
        ///         Always use <see cref="Bounds" /> (view-relative) when calling <see cref="Redraw(Rect)" />, NOT
        ...
        public virtual void Redraw(Rect bounds)
        {
            ...
            if (!ustring.IsNullOrEmpty(Text))
            {
                ...
                textFormatter?.Draw(ViewToScreen(Bounds), HasFocus ? ColorScheme.Focus : ColorScheme.Normal,
                    HasFocus ? ColorScheme.HotFocus : ColorScheme.HotNormal);
            }
                ...
                foreach (var view in subviews)
                    if (!view.NeedDisplay.IsEmpty || view.ChildNeedsDisplay)
                    {
                            ...
                            (view.Frame.IntersectsWith(bounds) || bounds.X < 0 || bounds.Y < 0))
                        {
                            ...
                            if (view.Visible) view.Redraw(view.Bounds);
                        }
        ...
        public override bool OnKeyDown(KeyEvent keyEvent)
        {
            ...
            if (Focused?.OnKeyDown(keyEvent) == true) return true;
        ...
        public override bool OnKeyUp(KeyEvent keyEvent)
        {
            ...
            KeyUp?.Invoke(args);
            if (args.Handled) return true;
            if (Focused?.OnKeyUp(keyEvent) == true) return true;
        ...
        /// <summary>
        ///     Sets the View's <see cref="Frame" /> to the relative coordinates if its container, given the <see cref="Frame" />
        ///     for its container.
        /// </summary>
        ...
        internal void SetRelativeLayout(Rect hostFrame)
        {
            ...
            if (Frame != r) Frame = new Rect(_x, _y, w, h);
        }
        ...
        private List<View> TopologicalSort(HashSet<View> nodes, HashSet<(View From, View To)> edges)
        {
            ...
            while (S.Any())
            {
                ...
                foreach (var e in edges.Where(e => e.From.Equals(n)).ToArray())
                {
                    ...
                    if (edges.All(me => me.To.Equals(m) == false) && m != this?.SuperView) // insert m into S
                        S.Add(m);
                }
            }
            ...
            if (edges.Any())
            {
                if (!ReferenceEquals(edges.First().From, edges.First().To))
                    throw new InvalidOperationException(
                        $"TopologicalSort (for Pos/Dim) cannot find {edges.First().From}. Did you forget to add it to {this}?");
                throw new InvalidOperationException(
                    "TopologicalSort encountered a recursive cycle in the relative Pos/Dim in the views of " + this);
            }
        ...
        public virtual void LayoutSubviews()
        {
            ...
            if (SuperView == Application.Top && LayoutNeeded && ordered.Count == 0 &&
                LayoutStyle == LayoutStyle.Computed) SetRelativeLayout(Frame);
        ...
        private bool ResizeView(bool autoSize)
        {
            if (textFormatter.Size != Bounds.Size && ((width == null || width is Dim.DimAbsolute) && (Bounds.Width == 0
                                                      ...
                                                      || (height == null || height is Dim.DimAbsolute) &&
                                                      (Bounds.Height == 0
        ...
        /// </summary>
        protected void OnMouseClick(MouseEventArgs args)
        {
        ...
        public class FocusEventArgs : EventArgs
        {
            ...
            ///     event subscriber.
            ///     Its important to set this value to true specially when updating any View's layout from inside the subscriber
            ///     method.
            /// </summary>
