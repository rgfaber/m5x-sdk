...
namespace M5x.Tty.Windows
{
	...
	public static class MessageBox
    {
        ...
        /// <remarks>
        ///     Use <see cref="Query(ustring, ustring, ustring[])" /> instead; it automatically sizes the MessageBox based on the
        ///     contents.
        /// </remarks>
        ...
        /// <param name="buttons">Array of buttons to add.</param>
        /// <remarks>
        ///     Use <see cref="ErrorQuery(ustring, ustring, ustring[])" /> instead; it automatically sizes the MessageBox based on
        ///     the contents.
        /// </remarks>
        ...
        private static int QueryFull(bool useErrorColors, int width, int height, ustring title, ustring message,
            params ustring[] buttons)
        {
            const int defaultWidth = 50;
            var textWidth = TextFormatter.MaxWidth(message, width == 0 ? defaultWidth : width);
            var textHeight = TextFormatter.MaxLines(message, textWidth); // message.Count (ustring.Make ('\n')) + 1;
            ...
            var msgboxWidth = Math.Max(defaultWidth,
                Math.Max(title.RuneCount + 8,
                    Math.Max(textWidth + 4, d.GetButtonsWidth()) +
                    8)); // textWidth + (left + padding + padding + right)
            d.Width = msgboxWidth;
