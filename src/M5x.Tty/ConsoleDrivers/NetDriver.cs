...
namespace M5x.Tty.ConsoleDrivers
{
    internal class NetDriver : ConsoleDriver
    {
        ...
        private void UpdateOffscreen()
        {
            ...
            {
                contents[row, c, 0] = ' ';
                contents[row, c, 1] = Colors.TopLevel.Normal;
                contents[row, c, 2] = 0;
                dirtyLine[row] = true;
            }
        }
        ...
        public override void Move(int col, int row)
        {
            ...
            else
            {
                if (cols == Console.WindowWidth && rows == Console.WindowHeight)
                {
                    if (Console.WindowHeight > 0) Console.SetCursorPosition(Clip.X, Clip.Y);
                    needMove = true;
                }
            }
        }
        ...
        private void SetColor(int color)
        {
            ...
            var values = Enum.GetValues(typeof(ConsoleColor))
                .OfType<ConsoleColor>()
                .Select(s => (int) s);
            if (values.Contains(color & 0xffff)) Console.BackgroundColor = (ConsoleColor) (color & 0xffff);
        ...
        public override void UpdateScreen()
        {
            ...
            for (var row = top; row < rows; row++)
            {
                ...
                for (var col = 0; col < cols; col++)
                {
                    ...
                    if (Console.WindowHeight > 0) Console.SetCursorPosition(col, row);
        ...
        private Key MapKey(ConsoleKeyInfo keyInfo)
        {
            ...
            if (key >= ConsoleKey.F1 && key <= ConsoleKey.F12)
            {
                ...
                if ((keyInfo.Modifiers & (ConsoleModifiers.Shift | ConsoleModifiers.Alt | ConsoleModifiers.Control)) !=
                    0) return (Key) ((uint) Key.F1 + delta);

                return (Key) ((uint) Key.F1 + delta);
            }

            if (keyInfo.KeyChar != 0) return (Key) keyInfo.KeyChar;

            return (Key) 0xffffffff;
        }

        private void MapKeyModifiers(ConsoleKeyInfo keyInfo)
        {
    ...
    public class NetMainLoop : IMainLoopDriver
    {
        ...
        private readonly ManualResetEventSlim keyReady = new(false);
        private readonly ManualResetEventSlim waitForProbe = new(false);
        private readonly ManualResetEventSlim winChange = new(false);
        ...
        {
            if (consoleDriver == null) throw new ArgumentNullException("console driver instance must be provided.");
        ...
        private void WaitWinChange()
        {
            while (true)
                if (Console.WindowWidth != consoleDriver.Cols || Console.WindowHeight != consoleDriver.Rows
                                                              || Console.WindowTop !=
                                                              consoleDriver.Top) // Top only working on Windows.
                    return;
        }
