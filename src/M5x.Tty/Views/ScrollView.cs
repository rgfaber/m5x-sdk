...
namespace M5x.Tty.Views
{
    ...
    public class ScrollBarView : View
    {
        ...
        /// <summary>
        ///     Initializes a new instance of the <see cref="ScrollBarView" /> class using <see cref="LayoutStyle.Absolute" />
        ///     layout.
        /// </summary>
        ...
        /// <summary>
        ///     Initializes a new instance of the <see cref="ScrollBarView" /> class using <see cref="LayoutStyle.Computed" />
        ///     layout.
        /// </summary>
        ...
        ///     part of a <see cref="ScrollView" /> the Size is set to the appropriate dimension of
        ///     <see cref="ScrollView.ContentSize" />.
        /// </remarks>
        public int Size
        {
            get => size;
        ...
        public override void Redraw(Rect region)
        {
            ...
            if (vertical)
            {
                ...
                else
                {
                    bh -= 2;
                    var by1 = position * bh / Size;
                    var by2 = Host.KeepContentAlwaysInViewport
                        ? Math.Min((position + bh) * bh / Size + 1, bh - 1)
                        : (position + bh) * bh / Size;
                    if (Host.KeepContentAlwaysInViewport && by1 == by2) by1 = Math.Max(by1 - 1, 0);
            ...
            else
            {
                ...
                else
                {
                    bw -= 2;
                    var bx1 = position * bw / Size;
                    var bx2 = Host.KeepContentAlwaysInViewport
                        ? Math.Min((position + bw) * bw / Size + 1, bw - 1)
                        : (position + bw) * bw / Size;
                    if (Host.KeepContentAlwaysInViewport && bx1 == bx2) bx1 = Math.Max(bx1 - 1, 0);
        ...
        public override bool MouseEvent(MouseEvent me)
        {
            ...
            var posTopLeftTee = vertical ? posTopTee : posLeftTee;
            var posBottomRightTee = vertical ? posBottomTee : posRightTee;
            ...
            {
                if (Host.CanScroll(1, out _, vertical)) SetPosition(pos + 1);
            }
            else if (location > 0 && location < barsize + 1)
            {
                var b1 = pos * barsize / Size;
                var b2 = Host.KeepContentAlwaysInViewport
                    ? Math.Min((pos + barsize) * barsize / Size + 1, barsize - 1)
                    : (pos + barsize) * barsize / Size;
                if (Host.KeepContentAlwaysInViewport && b1 == b2) b1 = Math.Max(b1 - 1, 0);

                if (location > b2 + 1 && location > posTopLeftTee && location > b1 && location > posBottomRightTee &&
                    posBottomRightTee > 0)
                {
                    Host.CanScroll(location, out var nv, vertical);
                    if (nv > 0) SetPosition(Math.Min(pos + nv, Size));
                }
                else if (location <= b1)
                {
    ...
    public class ScrollView : View
    {
        ...
        /// <summary>
        ///     Initializes a new instance of the <see cref="ScrollView" /> class using <see cref="LayoutStyle.Absolute" />
        ///     positioning.
        /// </summary>
        ...
        /// <summary>
        ///     Initializes a new instance of the <see cref="ScrollView" /> class using <see cref="LayoutStyle.Computed" />
        ///     positioning.
        /// </summary>
        public ScrollView()
        {
            ...
            set
            {
                if (keepContentAlwaysInViewport != value)
                {
                    ...
                    if (value && -contentOffset.X + Bounds.Width > contentSize.Width)
                        p = new Point(contentSize.Width - Bounds.Width + (showVerticalScrollIndicator ? 1 : 0),
                            -contentOffset.Y);
                    if (value && -contentOffset.Y + Bounds.Height > contentSize.Height)
                    {
                        if (p == default)
                            p = new Point(-contentOffset.X,
        ...
        private void View_MouseEnter(MouseEventArgs e)
        {
        ...
        {
            foreach (var view in contentView.Subviews) view.SetNeedsDisplay();
        }
        ...
        {
            var size = isVertical ? KeepContentAlwaysInViewport
                    ? Bounds.Height + (showHorizontalScrollIndicator ? -2 : -1)
                    : 0 :
            ...
            if (cSize < newSize - size) return true;
            return false;
        }
