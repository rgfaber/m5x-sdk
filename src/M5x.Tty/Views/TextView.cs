...
namespace M5x.Tty.Views
{
    ...
    {
        private readonly TextModel model = new();
        private int topRow;
        ...
        public override void Redraw(Rect bounds)
        {
            ...
            for (var row = bounds.Top; row < bottom; row++)
            {
                ...
                for (var col = bounds.Left; col < right; col++)
                {
                    ...
                    if (!SpecialRune(rune)) AddRune(col, row, rune);
                }
            }
        ...
        private bool SpecialRune(Rune rune)
        {
            switch (rune)
            {
                case (uint) Key.Enter:
