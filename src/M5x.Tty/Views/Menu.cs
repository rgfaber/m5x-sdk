...
namespace M5x.Tty.Views
{
    ...
    public class MenuItem
    {
        ...
        public MenuItem(ustring title, ustring help, Action action, Func<bool> canExecute = null,
            MenuItem parent = null, Key shortcut = Key.Null)
        {
            Title = title ?? "";
            Help = help ?? "";
            Action = action;
        ...
        /// <summary>
        ///     This is the global setting that can be used as a global <see cref="ShortcutHelper.Shortcut" /> to invoke the action
        ///     on the menu.
        /// </summary>
        public Key Shortcut
        {
        ...
        internal int Width => Title.RuneCount + Help.RuneCount + 1 + 2 +
                              (Checked || CheckType.HasFlag(MenuItemCheckStyle.Checked) ||
    ...
    public class MenuBarItem : MenuItem
    {
        ...
        public MenuBarItem(ustring title, ustring help, Action action, Func<bool> canExecute = null,
        ...
        private int GetMenuBarItemLength(ustring title)
        {
            var len = 0;
    ...
    internal class Menu : View
    {
        ...
        {
            if (items == null || items.Length == 0) return new Rect();
        ...
        public override void Redraw(Rect bounds)
        {
            ...
            for (var i = 0; i < barItems.Children.Length; i++)
            {
                ...
                Driver.SetAttribute(item == null ? ColorScheme.Normal :
        ...
        public override bool ProcessKey(KeyEvent kb)
        {
            switch (kb.Key)
            {
                ...
                case Key.CursorRight:
                    host.NextMenu(barItems.IsTopLevel || barItems.Children != null && current > -1 &&
    ...
    {
        private static ustring shortcutDelimiter = "+";
        ...
        /// <summary>
        ///     Gets or sets the array of <see cref="MenuBarItem" />s for the menu. Only set this when the <see cref="MenuBar" />
        ///     is vislble.
        /// </summary>
        ...
        public static ustring ShortcutDelimiter
        {
            ...
            set
            {
                if (shortcutDelimiter != value) shortcutDelimiter = value == ustring.Empty ? " " : value;
            }
        }
        ...
        public override bool OnKeyUp(KeyEvent keyEvent)
        {
            if (keyEvent.IsAlt)
            {
                ...
                if (!keyEvent.IsCtrl && openedByAltKey && !IsMenuOpen && openMenu == null &&
                    ((uint) keyEvent.Key & (uint) Key.CharMask) == 0)
                {
        ...
        public override void Redraw(Rect bounds)
        {
            ...
            for (var i = 0; i < Menus.Length; i++)
            {
                ...
                DrawHotString(menu.Help.IsEmpty ? $" {menu.Title}  " : $" {menu.Title}  {menu.Help}  ", hotColor,
                    normalColor);
            ...
            switch (subMenu)
            {
                ...
                default:
                    ...
                    else
                    {
                        ...
                        openCurrentMenu = new Menu(this, last.Frame.Left + last.Frame.Width,
                            last.Frame.Top + 1 + last.current, subMenu);
                    ...
                    if (selectedSub > -1 && SelectEnabledItem(openCurrentMenu.barItems.Children,
                        openCurrentMenu.current, out openCurrentMenu.current)) openCurrentMenu.SetFocus();
                    break;
            }
        ...
        public void OpenMenu()
        {
            ...
            OpenMenu(selected);
            if (!SelectEnabledItem(openCurrentMenu.barItems.Children, openCurrentMenu.current,
        ...
        internal void PreviousMenu(bool isSubMenu = false)
        {
            switch (isSubMenu)
            {
                case false:
                    ...
                    OpenMenu(selected);
                    if (!SelectEnabledItem(openCurrentMenu.barItems.Children, openCurrentMenu.current,
                        out openCurrentMenu.current, false))
                    {
            ...
            switch (isSubMenu)
            {
                case false:
                    ...
                    if (selected > -1)
                        CloseMenu(true);
                    OpenMenu(selected);
                    SelectEnabledItem(openCurrentMenu.barItems.Children, openCurrentMenu.current,
                        out openCurrentMenu.current);
                    break;
                case true:
                    ...
                    else
                    {
                        var subMenu =
                            openCurrentMenu.barItems.SubMenu(
                                openCurrentMenu.barItems.Children[openCurrentMenu.current]);
        ...
        public override bool ProcessHotKey(KeyEvent kb)
        {
            ...
            if (kb.IsAlt && !kb.IsCtrl && !kb.IsShift)
                if (FindAndOpenMenuByHotkey(kb))
                    return true;
            //var kc = kb.KeyValue;
