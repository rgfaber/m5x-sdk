...
namespace M5x.Consul.Utilities
{
    public static class Extensions
    {
        ...
        public static string ToGoDuration(this TimeSpan ts)
        {
            ...
            if (ts.TotalSeconds < 1) return ts.TotalMilliseconds.ToString("#ms");

            var outDuration = new StringBuilder();
            if ((int)ts.TotalHours > 0) outDuration.Append(ts.TotalHours.ToString("#h"));
            if (ts.Minutes > 0) outDuration.Append(ts.ToString("%m'm'"));
            if (ts.Seconds > 0) outDuration.Append(ts.ToString("%s"));
            ...
            if (ts.Seconds > 0) outDuration.Append("s");
        ...
        internal static TimeSpan FromGoDuration(string value)
        {
            ...
            if (string.IsNullOrEmpty(value) || value == "0") return TimeSpan.Zero;

            if (ulong.TryParse(value, out var result)) return TimeSpan.FromTicks((long)(result / 100));

            var matches = Regex.Matches(value, pattern);
            if (matches.Count == 0) throw new ArgumentException("Invalid duration", value);
            double time = 0;

            foreach (Match match in matches)
                if (double.TryParse(match.Groups[1].Value, out var res))
