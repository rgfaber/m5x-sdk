...
namespace M5x.DEC.Core
{
    public class MetadataContainer : Dictionary<string, string>
    {
        ...
        {
            foreach (var keyValuePair in keyValuePairs) Add(keyValuePair.Key, keyValuePair.Value);
        }
        ...
        public T GetMetadataValue<T>(string key, Func<string, T> converter)
        {
            ...
            if (!TryGetValue(key, out value)) throw new MetadataKeyNotFoundException(key);
