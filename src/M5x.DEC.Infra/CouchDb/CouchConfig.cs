...
{
    public static string CouchEndPoint =
        Environment.GetEnvironmentVariable(EnVars.COUCH_ENDPOINT) ?? "http://db.local:5984";

    public static string CouchUser = Environment.GetEnvironmentVariable(EnVars.COUCH_USER) ?? "root";
    public static string CouchPwd = Environment.GetEnvironmentVariable(EnVars.COUCH_PWD) ?? "logatron";
