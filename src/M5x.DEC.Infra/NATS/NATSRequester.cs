...
{
    public abstract class NATSRequester<TRequest, TRsp> : IRequester<TRequest, TRsp>
        ...
        where TRsp : Response, new()
    {
        ...
        protected NATSRequester(INatsClient bus, ILogger logger)
        {
            _bus = bus;
            _logger = logger;
        }
            ...
            
        {
            ...
            try
            {
                ...
                _logger?.Debug($"::REQUEST command: {req.CorrelationId} on topic {Topic}.");
                var msg = await _bus.RequestAsync(Topic, req.NatsEncode());
                _logger?.Debug($"::RECEIVED response: {msg.SubscriptionId} on {msg.ReplyTo}.");
                rsp = msg?.Payload.NatsDecode<TRsp>();
            }
            catch (Exception e)
            {
