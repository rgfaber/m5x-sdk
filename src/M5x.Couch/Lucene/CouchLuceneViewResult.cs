...
{
    private CouchLuceneViewDefinition _view;
    public string Etag;
    ...
    public virtual IEnumerable<CouchLuceneHit> Hits()
    {
        ...
        foreach (JObject row in Rows()) hits.Add(new CouchLuceneHit(row));
        return hits;
    }
    ...
    public virtual IEnumerable<T> GetDocuments<T>() where T : ICouchDocument, new()
    {
        ...
        var hits = Hits();
        if (!hits.Any()) return docs;

        var firstHit = hits.First();
        var db = _view.Db();
        if (firstHit.HasDocument())
        {
            foreach (var hit in hits) docs.Add(hit.Document<T>());
            return docs;
        }

        var ids = new List<string>();
        foreach (var hit in hits) ids.Add(hit.Id());
