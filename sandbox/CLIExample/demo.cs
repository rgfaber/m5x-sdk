...
namespace Example
{
    internal static class Demo
    {
		...
		scrollView.Add (new Box10x (0, 0));
            ...
            scrollView.DrawContent += r => { scrollView.ContentSize = filler.GetContentSize(); };
#endif
            ...
            container.Add(
                ...
                passText,
                frameView,
                new ListView(new Rect(59, 6, 16, 4), new[]
                {
                    "First row",
                    "<>",
        ...
        public static void Open()
        {
            ...
            if (!d.Canceled)
                MessageBox.Query(50, 7, "Selected File",
                    d.FilePaths.Count > 0 ? string.Join(", ", d.FilePaths) : d.FilePath, "Ok");
        }
        ...
        public static void ShowHex()
        {
            ...
            var source = File.OpenRead(fname);
        ...
        public static void ShowMenuItem(MenuItem mi)
        {
            ...
            var minfo = typeof(MenuItemDetails).GetMethod("Instance", flags);
            var mid = (MenuItemDelegate) Delegate.CreateDelegate(typeof(MenuItemDelegate), minfo);
            MessageBox.Query(70, 7, mi.Title.ToString(),
                $"{mi.Title} selected. Is from submenu: {mi.GetMenuBarItem()}", "Ok");
        }

        private static void MenuKeysStyle_Toggled(bool e)
        {
            menu.UseKeysUpDownAsKeysLeftRight = menuKeysStyle.Checked;
        }

        private static void MenuAutoMouseNav_Toggled(bool e)
        {
        ...
        {
            var textField = menu.LastFocused as TextField ?? Application.Top.MostFocused as TextField;
        ...
        private static void Main()
        {
            ...
            while (running != null) running.Invoke();
		...
		var win = new Window (new Rect (0, 1, tframe.Width, tframe.Height - 1), "Hello");
            ...
            menu = new MenuBar(new[]
            {
                new MenuBarItem("_File", new[]
                {
                    new("Text _Editor Demo", "", () =>
                    {
                        running = Editor;
                        Application.RequestStop();
                    }, null, null, Key.AltMask | Key.CtrlMask | Key.D),
                    ...
                    new MenuItem("_Hex", "", () =>
                    {
                        running = ShowHex;
                        Application.RequestStop();
                    }, null, null, Key.AltMask | Key.CtrlMask | Key.H),
                ...
                new MenuBarItem("_List Demos", new[]
                {
                    new MenuItem("Select _Multiple Items", "", () => ListSelectionDemo(true), null, null,
                        Key.AltMask + 0.ToString()[0]),
                    new MenuItem("Select _Single Item", "", () => ListSelectionDemo(false), null, null,
                        Key.AltMask + 1.ToString()[0]),
                    new MenuItem("Search Single Item", "", ComboBoxDemo, null, null, Key.AltMask + 2.ToString()[0])
                }),
                new MenuBarItem("A_ssorted", new[]
                {
                    new MenuItem("_Show text alignments", "", () => ShowTextAlignments(), null, null,
                        Key.AltMask | Key.CtrlMask | Key.G),
                    new MenuItem("_OnKeyDown/Press/Up", "", () => OnKeyDownPressUpDemo(), null, null,
                        Key.AltMask | Key.CtrlMask | Key.K)
                }),
                new MenuBarItem("_Test Menu and SubMenus", new[]
                {
                    new MenuBarItem("SubMenu1Item_1", new[]
                    {
                        new MenuBarItem("SubMenu2Item_1", new[]
                        {
                ...
                new MenuBarItem("_About...", "Demonstrates top-level menu item",
                    () => MessageBox.ErrorQuery(50, 7, "About Demo", "This is a demo app for gui.cs", "Ok"))
            });
        ...
        private static void ListSelectionDemo(bool multiple)
        {
            var ok = new Button("Ok", true);
            ok.Clicked += () => { Application.RequestStop(); };
            var cancel = new Button("Cancel");
        ...
        private static void ComboBoxDemo()
        {
            ...
            list.OpenSelectedItem += text => { Application.RequestStop(); };

            var d = new Dialog {Title = "Select source file", Width = Dim.Percent(50), Height = Dim.Percent(50)};
            d.Add(list);
            Application.Run(d);

            MessageBox.Query(60, 10, "Selected file",
                list.Text.ToString() == "" ? "Nothing selected" : list.Text.ToString(), "Ok");
        }
