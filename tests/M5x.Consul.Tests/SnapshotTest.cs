...
{
    public class SnapshotTest : ConsulTestsBase, IDisposable
    {
        public SnapshotTest(ITestOutputHelper output, IoCTestContainer container) : base(output, container)
        {
        }
        ...
        //        await Assert.ThrowsAsync<ConsulRequestException>(() =>
        //            client.Snapshot.Save(new QueryOptions {Token = "anonymous"}));

        //        // Now try an unknown DC.
        //        await Assert.ThrowsAsync<ConsulRequestException>(() =>
        //            client.Snapshot.Save(new QueryOptions {Datacenter = "nope"}));
        ...
        //        var snapStale = await client.Snapshot.Save(new QueryOptions
        //        {
        //            Token = ACLTest.ConsulRoot,
        ...
        //        await Assert.ThrowsAsync<ConsulRequestException>(() =>
        //            client.Snapshot.Restore(new MemoryStream(snapData, false), new WriteOptions {Token = "anonymous"}));

        //        // Now try an unknown DC.
        //        await Assert.ThrowsAsync<ConsulRequestException>(() =>
        //            client.Snapshot.Restore(new MemoryStream(snapData, false), new WriteOptions {Datacenter = "nope"}));

        //        // This should work.
        //        await client.Snapshot.Restore(new MemoryStream(snapData, false),
        //            new WriteOptions {Token = ACLTest.ConsulRoot});
        //    }
        //}
        ...
        {
            using (var client = new ConsulClient.ConsulClient(c => { c.Token = ACLTest.ConsulRoot; }))
            {
