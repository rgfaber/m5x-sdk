...
{
    [Trait("speed", "slow")]
    public class SemaphoreTest : ConsulTestsBase, IDisposable
    {
        public SemaphoreTest(ITestOutputHelper output, IoCTestContainer container) : base(output, container)
        {
        }
        ...
        public async Task Semaphore_ContendFast()
        {
            ...
            {
                cts.CancelAfter((contenderPool - 1) *
                                (int) Semaphore.Semaphore.DefaultSemaphoreWaitTime.TotalMilliseconds);
        ...
        public async Task Semaphore_DeleteKey()
        {
            ...
            var semaphore = (Semaphore.Semaphore) client.Semaphore(keyName, 2);
            ...
            try
            {
                ...
                {
                    while (semaphore.IsHeld) Thread.Sleep(10);
        ...
        //{
        //    var c = new ConsulClient.ConsulClient();
        ...
        public async Task Semaphore_OneShot()
        {
            ...
            Assert.Equal(Semaphore.Semaphore.DefaultSemaphoreWaitTime, semaphoreOptions.SemaphoreWaitTime);
            ...
            Task.WaitAny(
                ...
                Task.Delay((int) (2 * semaphoreOptions.SemaphoreWaitTime.TotalMilliseconds))
                    .ContinueWith(t => Assert.True(false, "Took too long"))
            );
