...
{
    [Trait("speed", "slow")]
    public class LockTest : ConsulTestsBase, IDisposable
    {
        public LockTest(ITestOutputHelper output, IoCTestContainer container) : base(output, container)
        {
        }
        ...
        public async Task Lock_Contend_LockDelay()
        {
            ...
            var acquired = new ConcurrentDictionary<int, bool>();
            using (var cts = new CancellationTokenSource())
            {
                cts.CancelAfter((contenderPool + 1) * (int) Lock.Lock.DefaultLockWaitTime.TotalMilliseconds);
                ...
                for (var i = 0; i < contenderPool; i++)
                {
                    ...
                    {
                        var lockKey = (Lock.Lock) client.CreateLock(keyName);
                        await lockKey.Acquire(CancellationToken.None);
                        if (lockKey.IsHeld)
                        {
        ...
        public async Task Lock_ContendFast()
        {
            ...
            using (var cts = new CancellationTokenSource())
            {
                cts.CancelAfter(contenderPool * (int) Lock.Lock.DefaultLockWaitTime.TotalMilliseconds);
                ...
                for (var i = 0; i < contenderPool; i++)
                {
                    ...
                    tasks.Add(Task.Run(async () =>
                    {
                        ...
                        await lockKey.Acquire(CancellationToken.None);
                        Assert.True(acquired.TryAdd(v, lockKey.IsHeld));
                        if (lockKey.IsHeld) await lockKey.Release();
                    }));
                }
        ...
        public async Task Lock_ContendWait()
        {
            ...
            for (var i = 0; i < contenderPool; i++)
                Assert.True(acquired[i], "Contender " + i + " did not acquire the lock");
        }
        ...
        public async Task Lock_DeleteKey()
        {
            ...
            try
            {
                ...
                {
                    while (lockKey.IsHeld) Thread.Sleep(10);
                    Assert.False(lockKey.IsHeld);
                });
        ...
        public async Task Lock_EphemeralAcquireRelease()
        {
            ...
            var l = await client.AcquireLock(new LockOptions(keyName) {Session = sessionId.Response},
        ...
        public async Task Lock_ForceInvalidate()
        {
            ...
            try
            {
                ...
                {
                    while (lockKey.IsHeld) await Task.Delay(10);
                    Assert.False(lockKey.IsHeld);
                });
        ...
        public void Lock_MultithreadedRelease()
        {
            ...
            using (var client = new ConsulClient.ConsulClient())
            {
                ...
                for (var i = 0; i < numTasks; i++) tasks[i] = lockKey.Acquire(CancellationToken.None);
                ...
                for (var i = 0; i < numTasks; i++) tasks[i] = lockKey.Release(CancellationToken.None);
        ...
        //            })
        //        },
        //        2 * lockOptions.LockWaitTime.TotalMilliseconds));
